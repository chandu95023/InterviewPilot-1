...
import { evaluateInterview, generateCompanyQuestions, getCompanies } from '../api/api'
...
const CompanyInterview = () => {
  ...
  const handleStartSession = async () => {
    ...
    setError('')
    try {
      const response = await generateCompanyQuestions({ 
      ...
      if (response.data.questions && response.data.questions.length > 0) {
        ...
        setError('No questions compiled for this loop setup. Please try again.')
      }
    } catch (err) {
      setError('Unable to compile company loop questions. Verify connections.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleEndInterview = async () => {
    ...
    try {
      const response = await evaluateInterview({
        domain: `${selectedCompany} loop: ${selectedDomain}`,
        difficulty,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-slate-900 pb-5">
        <h2 className="text-xl font-bold text-white tracking-wide">Company Round Simulator</h2>
        <p className="text-xs text-slate-400 mt-1">Simulate specific technical rounds based on curated questions from company hiring bars.</p>
      </div>
        ...
        >
          <div className="text-center space-y-2">
            <span className="inline-flex items-center gap-2 rounded-full bg-indigo-950/80 px-3.5 py-1.5 text-xs text-indigo-300 border border-indigo-900/30">
              ...
              Target Brand Simulator
            </span>
            <h1 className="text-2xl font-bold text-white tracking-wide">Company Interview Round</h1>
            <p className="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed">
              Generate 3 questions curated from target company loops. Solve under realistic timer pressure.
            </p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-350">Company</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-350">Domain</label>
              <select 
                className="w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-xs text-white outline-none focus:border-indigo-500" 
                value={selectedDomain} 
                onChange={(e) => setSelectedDomain(e.target.value)}
              >
                {['Java', 'Python', 'Full Stack Development', 'Data Science', 'AI/ML'].map((opt) => (
          ...
          >
            {loading ? (
              <>
                ...
                <span>Generating Company loop configurations...</span>
              </>
            ) : (
              <>
                <Play className="h-4.5 w-4.5" />
                <span>Begin Assessment Loop</span>
              </>
            )}
          </button>
        </motion.div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.8fr_1fr] items-stretch">
          ...
          <div className="glass-card rounded-[2rem] p-6 border border-slate-800 shadow-md flex flex-col justify-between space-y-6 bg-slate-950/40">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider block">Domain: {selectedDomain} ({selectedCompany})</span>
          ...
          <div className="flex flex-col gap-6">
            <div className="glass-card rounded-[2rem] border border-slate-805 bg-slate-950 p-8 shadow-md flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden">
              <div className="absolute top-4 left-4 bg-slate-900/90 backdrop-blur border border-slate-805 px-3 py-1.5 rounded-xl flex items-center gap-1.5">
                <Clock className="h-3.5 w-3.5 text-indigo-400" />
                <span className="text-[10px] font-bold text-white tracking-wider font-mono">{formatTime(timer)}</span>
              </div>

              <div className="flex flex-col items-center space-y-4 text-center">
                <div className={`h-20 w-20 rounded-full flex items-center justify-center p-0.5 ${isRecording ? 'bg-gradient-to-tr from-cyan-400 to-indigo-500 animate-pulse' : 'bg-slate-900 border border-slate-800'}`}>
                  <div className="h-full w-full rounded-full bg-slate-950 flex items-center justify-center">
                    <Mic className={`h-8 w-8 ${isRecording ? 'text-cyan-400 animate-bounce' : 'text-slate-500'}`} />
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-bold text-white">Interviewer Sim ({selectedCompany})</h4>
                  <p className="text-[9px] text-indigo-400 font-medium tracking-wide uppercase mt-0.5">{selectedCompany} Standard Round</p>
                </div>
              </div>
            </div>

            <div className="glass-card rounded-[2rem] p-5 border border-slate-800 shadow-md flex-1 space-y-3 bg-slate-950/40">
              <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block">Live speech transcription</span>
              <div className="rounded-xl border border-slate-900 bg-slate-950 p-4 h-24 overflow-y-auto text-xs leading-relaxed text-slate-400 font-mono">
                {speechTranscript || answers[currentIndex] || 'Answers will compile here. Speak or type responses...'}
              </div>
            </div>
          </div>
          ...
          <div className="glass-card rounded-[2rem] p-6 border border-slate-800 shadow-md space-y-6 bg-slate-950/40 flex flex-col justify-between">
            <div className="space-y-5">
              <span className="text-[10px] text-slate-550 font-bold uppercase tracking-wider block">Live Metrics</span>

              {/* Confidence */}
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] font-bold">
                  <span className="text-slate-400 flex items-center gap-1">
                    <TrendingUp className="h-3.5 w-3.5 text-indigo-400" /> Articulation Level
                  </span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[10px] font-bold">
                  <span className="text-slate-400 flex items-center gap-1">
                    <Eye className="h-3.5 w-3.5 text-violet-400" /> Gaze Focus
                  </span>
            ...
            <div className="rounded-xl border border-slate-900 bg-slate-950 p-4 text-center">
              <span className="text-[9px] uppercase font-bold text-slate-500 block tracking-widest font-mono">Company loop evaluation</span>
            </div>
          </div>
        </div>
      )}
          ...
          <button 
            ...
            className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold border transition ${
              isRecording 
                ...
                : 'bg-slate-900/45 border-slate-805 text-slate-350 hover:text-white'
            }`}
          >
          ...
          <div className="flex gap-3">
            {currentIndex < questions.length - 1 ? (
              <button 
                ...
                className="flex items-center gap-1.5 rounded-xl border border-slate-850 bg-slate-950 px-4 py-2.5 text-xs font-bold text-slate-350 hover:text-white transition"
              >
            ...
            >
              {evaluating ? (
                <>
                  <div className="h-4 w-4 rounded-full border-2 border-white border-t-transparent animate-spin" />
                  <span>Evaluating loop responses...</span>
                </>
              ) : (
                <>
        ...
        >
          <div className="text-center space-y-2">
            <span className="inline-flex items-center gap-2 rounded-full bg-emerald-950/80 px-3.5 py-1.5 text-xs text-emerald-400 border border-emerald-900/40">
              ...
              Company Round Complete
            </span>
            <h2 className="text-2xl font-bold text-white tracking-wide">AI Feedback Analysis</h2>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3 font-mono">
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-5 text-center">
              <span className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Brand Compatibility</span>
              <p className="text-4xl font-extrabold text-white mt-2">
                {(result.score ?? result.average_score ?? 8.2) * 10}%
              </p>
            </div>
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-5 sm:col-span-2 space-y-2">
              ...
              <p className="text-xs text-slate-350 leading-relaxed">
                {Array.isArray(result.strengths) 
                  ...
                  : `Solid alignments with engineering standard definitions, good runtime complexity descriptions.`
                }
              </p>
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-5 space-y-2">
              ...
              <p className="text-xs text-slate-355 leading-relaxed">
                {Array.isArray(result.weaknesses) 
                  ...
                  : 'Be more specific about trade-offs and describe multiple options when designing data collections.'
                }
              </p>
            </div>
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-5 space-y-2">
              ...
              <p className="text-xs text-slate-350 leading-relaxed">
                {Array.isArray(result.suggestions) 
                  ...
                  : 'Review system patterns for scale, and run mock scenarios focusing specifically on memory constraint limits.'
                }
              </p>
            </div>
          </div>
