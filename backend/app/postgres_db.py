...
DATABASE_URL = os.getenv("DATABASE_URL") or os.getenv("POSTGRESQL_URI", "sqlite:///./interview_prep.db")
...
def init_db():
    ...
    if engine:
        try:
            ...
            try:
                engine = create_engine("sqlite:///./interview_prep.db", connect_args={"check_same_thread": False})
                SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
                Base.metadata.create_all(bind=engine)
